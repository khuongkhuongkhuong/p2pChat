...
public class ChatHistoryStore {
    ...
    static {
        ...
        catch (IOException e) { System.err.println("Không tạo được thư mục lịch sử: " + e.getMessage()); }
    }
    ...
    public static List<String> getConversationKeys(String username) {
        ...
        try {
            ...
            for (File f : files) {
                // Đảo ngược: bỏ prefix và ".txt" → lấy phần key đã safe-encode
                String safePart = f.getName().substring(prefix.length(),
                        f.getName().length() - 4); // bỏ ".txt"
                // Khôi phục key gốc: peer_Alice → peer:Alice, group_123 → group:123
                String key = safePart.replaceFirst("^(peer|group|broadcast)_", "$1:");
    ...
    public static void clear(String username, String key) {
        try { Files.deleteIfExists(Paths.get(fileName(username, key))); }
        catch (IOException e) { System.err.println("Lỗi xóa lịch sử: " + e.getMessage()); }
    }
}
